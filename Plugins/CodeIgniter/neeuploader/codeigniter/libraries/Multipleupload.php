...
class Multipleupload {
	...
	public function  do_upload($file = 'userfile') {
		...
		foreach($this->FILES as $key => $val):
			...
			// for low level error reporting
			$this->_mAPIError = $_FILES[$file];
			...
			$config['new_image'] = $this->_mFields['upload_path'] . 'thumbs/' . $_FILES[$file]['name'];
			...
			//array_push($this->_mData,$this->CI->upload->data());
			$this->_mData = $this->CI->upload->data();
			$cntr++;
		endforeach;

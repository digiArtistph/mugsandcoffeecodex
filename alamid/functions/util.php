...
if(! function_exists('loadAlamidGlobals')) {
	function loadAlamidGlobals() {
		...
		$almd_globals = array();
		$almd_db = "testing database";
		

	}
}

if( ! function_exists( "ajax_nonce" ) )  {
	function ajax_nonce ( $params = array(), $create = true ) {
		global $almd_globals;

		$almd_globals[ 'nonce' ] = array();

		if ( $create ) {
			array_push( $almd_globals [ 'nonce' ] , $params );
		} else {
			$almd_globals[ 'nonce' ] = array();
		}
	}
...
if( ! function_exists("__ACTIVATION_DATA") ) {
	function __ACTIVATION_DATA ( $data = array(), $create = true ) {
		global $almd_globals;

			
		if( $create ){

			$almd_globals[ 'activation' ] = array();

			$config = array(
				"account" => "admin@wimtown.com",
				"status" => "failed",
				"msg" => "Sorry! Your activation has been failed."
				);

				$data = array_merge( $config, $data );

				$almd_globals[ 'activation' ] = $data;

		} else {
			if( array_key_exists("activation", $almd_globals))
				return 	$almd_globals;
			else
				return array();
...
/**
 * Stores page hits on the database
 * @param $page string
 * @param $inc int
 * @return null
 * @since Wednesday, July 9, 2014
 * @version 1.0.0
 */
if(!function_exists('page_hits')) {
	function page_hits($page, $inc = 1) {

		$CI =& get_instance();
		$Qry = sprintf("INSERT INTO modules (name, hit) VALUES ('%s', %d)  ON DUPLICATE KEY UPDATE hit = hit + %d;", strtolower($page), $inc, $inc);
		$CI->db->query($Qry);
	}	
}
...
function hash_id( $id ) {
	return sha1(strval( $id ));
	
}

function garage_title ( $title ) {
	$arr_container = preg_split("/,/", $title);

	if (count( $arr_container ) > 0 && count( $arr_container) == 1)
		return "{$arr_container[0]} Sale";
	else
		return "{$arr_container[0]} and {$arr_container[1]} Sale";
